...
def init_browser(url="about:blank", headless = True):
    user_agent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.53 Safari/537.36"
    config_params = webdriver.ChromeOptions()
    config_params.add_argument('--incognito')
    config_params.add_argument('--enable-unsafe-swiftshader')
    ...
    config_params.add_argument("--log-level=3")
    config_params.add_argument("disable-logging")
    
    config_params.add_experimental_option("excludeSwitches", ["enable-logging"])
